...
/* Parses ports */
static void
parse_ports(const char *arg, struct nf6_nat_range *r)
{
	char *end;
...
static void MASQUERADE_parse(struct xt_option_call *cb)
{
	...
	struct nf6_nat_range *r = cb->data;
	int portok;
...
MASQUERADE_print(const void *ip, const struct xt_entry_target *target,
                 int numeric)
{
	const struct nf6_nat_range *r = (const void *)target->data;
...
static struct xtables_target masquerade_tg_reg = {
	...
	.family		= NFPROTO_IPV6,
	.size		= XT_ALIGN(sizeof(struct nf6_nat_range)),
	.userspacesize	= XT_ALIGN(sizeof(struct nf6_nat_range)),
